...
doc.vec <- VectorSource(John) # Construct a Source of Vector as input
...
findFreqTerms(TDM,100)  #each of this words occurred more than 1000 times
...
x <- rbind(m)
x <-t(x)
result <- cmeans(x,7,35844,verbose = TRUE,method = "cmeans")
print(result)
s3d <- scatterplot3d(result$membership, color = result$cluster, type = "h", angle = 55, scale.y = 0.7, pch = 16, main = "Pertinence for Fuzzy Amazon Dataset File 5")
