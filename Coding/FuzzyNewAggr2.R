...
newsCorpora <- read_csv("~/Masters Projet/News Aggregator/newsCorpora.csv", 
                        col_names = FALSE)
## News Aggregator Dataset File 2
length(newsCorpora)
head(newsCorpora)
tail(newsCorpora)
summary(newsCorpora)
...
x <- rbind(m)
x <-t(x)
result <- cmeans(x,4,50,verbose = TRUE,method = "cmeans")
print(result)
s3d <- scatterplot3d(result$membership, color = result$cluster, type = "h", angle = 55, scale.y = 0.7, pch = 16, main = "Pertinence for Fuzzy News Aggregator Dataset File 2")
